...
I am interested in school attendance data and found an interesting data set on the Florida Department of Education Website. This seemed like a good opportunity to learn the ` r readxl package `. This post closely follows closely with the examples on the [Github Repo](https://github.com/tidyverse/readxl). 

The file consists of results of a survey of schools in Florida that captured the count of students who were absent more than 21 days in the  2015-2016 school year. 
...
The output is not in a tidy format. The first row has a title and the second row provides a description of the data. Lets extract just the meta data using ` r (n_max = )` argument.  
...
This is not exactly what I want. Using the ` r (n_max =)` function  we captured the title and message about the data, but we also captured the other columns and row information.  I am going to use the  ` r (range)` argument to isolate the title and description of the data. 
...
Now, that we have the title and message about the data, let us put the data in tidy format. From our exercise above we know that the first two rows are not helpful. Fortunately we can use the ` r (skip = )` argument to bypass the first 2 rows. 
